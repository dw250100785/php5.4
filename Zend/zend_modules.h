...
/*
 * 模块实体
 * 分别定义了模块的名称，依赖模块， 各种相应方法，以及全局变量及其建立销毁方法
 */
struct _zend_module_entry {
	// 以下六个属性为标准模块头相关的
	unsigned short size;                                     // 模块实体的尺寸
	unsigned int zend_api;                                   // Zend api版本编号
	unsigned char zend_debug;                                // debug标志
	unsigned char zts;                                       // zend thread safety
	const struct _zend_ini_entry *ini_entry;                 // ini实例数组 
	const struct _zend_module_dep *deps;                     // 依赖模块数组
	
	const char *name;                                        // 模块实体名称
	const struct _zend_function_entry *functions;            // 模块实体函数集
	...
	void (*info_func)(ZEND_MODULE_INFO_FUNC_ARGS);           // 扩展信息方法
	const char *version;                                     // 版本号

	// 以下为全局变量的范围，可通过ZEND_MODULE_GLOBALS(extname)来替换
	size_t globals_size;                                     // 全局变量的尺寸
#ifdef ZTS
	ts_rsrc_id* globals_id_ptr;                              // ZTS时候的全局id指针
#else
	void* globals_ptr;                                       // 非ZTS时的全局变量指针
#endif
	void (*globals_ctor)(void *global TSRMLS_DC);            // 全局变量创建方法 ZEND_MODULE_GLOBALS_CTOR_N(extname), may be NULL
	void (*globals_dtor)(void *global TSRMLS_DC);            // 全局变量销毁方法 ZEND_MODULE_GLOBALS_DTOR_N(extname), may be NULL
	int (*post_deactivate_func)(void);                       // deactivate之后的处理方法
	
	// 以下五个属性为标准模块属性相关
	int module_started;                                      // 模块是否启动
	unsigned char type;                                      // 类型
	void *handle;                                            // 处理器
	int module_number;                                       // 模块数量
	const char *build_id;                                    // 构建ID
};
