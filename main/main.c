...
{
	zend_utility_functions zuf;                      // 工具函数，包含了错误, 打印, open, message等方法的结构体
	zend_utility_values zuv;                         // 导入使用的扩展及错误信息记录的结构体变量
	...
	void ***tsrm_ls;
	php_core_globals *core_globals;                  // 对应于PHP的配置， 核心全局配置
#endif
	...
	module_shutdown = 0;
	module_startup = 1;
	sapi_initialize_empty_request(TSRMLS_C);           // SAPI初始化，使用空的请求, 请求上下文，请求信息，请求方法等均初始化为空值
	sapi_activate(TSRMLS_C);
