...
const opt_struct OPTIONS[] = {
	{'a', 0, "interactive"},          // 交互模式
	{'B', 1, "process-begin"},        // 在处理输入行之前运行的代码，该选项后面跟的是具体php代码
	{'C', 0, "no-chdir"}, /* for compatibility with CGI (do not chdir to script directory) */
	{'c', 1, "php-ini"},              // 在给定的路径下面查找php.ini文件，后面跟一个目录或者文件
	{'d', 1, "define"},               // 定义ini实体，eg: -d foo=bar 
	{'E', 1, "process-end"},          // 在处理完所有输入行之后执行该选项后面的代码, 该选项后面跟的是具体的php代码
	{'e', 0, "profile-info"},         // 为debugger/profiler产生扩展信息
	{'F', 1, "process-file"},         // 表示处理的是脚本文件的代码，该选项带一个文件路径的参数
	{'f', 1, "file"},                 // 解析执行文件，选项后面跟具体脚本文件作为参数
	{'h', 0, "help"},                 // 显示帮助信息
	{'i', 0, "info"},                 // php信息
	{'l', 0, "syntax-check"},         // 只进行语法检查
	{'m', 0, "modules"},              // 显示以模块的形式编译
	{'n', 0, "no-php-ini"},           // 不使用php.ini
	{'q', 0, "no-header"}, /* for compatibility with CGI (do not generate HTTP headers) */
	{'R', 1, "process-code"},         // 处理代码，该选项后面跟具体的要处理的代码
	{'H', 0, "hide-args"},            // 隐藏所有来自外部工具的传入参数
	{'r', 1, "run"},                  // 不使用<? ... ?>标记运行php代码
	{'s', 0, "syntax-highlight"},     // 输出HTML语法高亮源代码
	{'s', 0, "syntax-highlighting"},  // 
	{'S', 1, "server"},               // 使用内建的web服务器运行，该选项后面跟<addr>:<port>作为内建服务器地址及端口号
	{'t', 1, "docroot"},              // 为内置的web服务器指定docroot
	{'w', 0, "strip"},                // 输出去掉注释和空格的源代码
	{'?', 0, "usage"},/* help alias (both '?' and 'usage') */
	{'v', 0, "version"},              // 版本号
	{'z', 1, "zend-extension"},       // 加载zend扩展文件, 选项后面跟具体的zend扩展文件名
	{10,  1, "rf"},                   // 显示给定名称的函数的信息,用法如: php --rf array_search
	{10,  1, "rfunction"},            // 同上
	{11,  1, "rc"},                   // 显示给定名称的类的信息, 用法类似上面的函数
	{11,  1, "rclass"},               // 同上
	{12,  1, "re"},                   // 显示给定名称扩展的信息，用法类似
	{12,  1, "rextension"},           // 同上
	{13,  1, "rz"},                   // 显示给定名称的zend扩展的信息
	{13,  1, "rzendextension"},       // 同上
	{14,  1, "ri"},                   // 显示给定名称扩展的配置信息
	{14,  1, "rextinfo"},             // 同上
	{15,  0, "ini"},                  // 显示配置文件列表
	{'-', 0, NULL} /* end of args */
};
